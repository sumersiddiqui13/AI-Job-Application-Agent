...
function page(records) {
  const cards = records.map((record) => `
    <article class="card">
      <div class="top"><div><h2>${escapeHtml(record.title || 'Untitled role')}</h2><p>${escapeHtml(record.company || '')} · ${escapeHtml(record.location || '')}</p></div><strong>${escapeHtml(record.matchScore ?? 0)}%</strong></div>
      <p><a href="${escapeHtml(record.url || '#')}" target="_blank" rel="noreferrer">Open job</a></p>
